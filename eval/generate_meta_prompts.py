...
def generate_meta_prompt_for_category(
    category: str, category_data: pd.DataFrame
) -> Dict[str, Any]:
    """
    ...
    for idx, row in category_data.head(3).iterrows():
        ...
        sample_task_details.append(
            {"prompt": str(row["prompt"]), "reference_files": ref_files}
        )
    ...
    try:
        ...
        log_message(
            f"Successfully generated meta-prompt for {category} (tokens: {response.usage.total_tokens})"
        )
        return result
    ...
    for i, category in enumerate(categories, 1):
        ...
        if category_already_generated(category, output_dir):
            ...
            try:
                ...
                log_message(
                    f"Warning: Could not load existing file for {category}: {str(e)}"
                )
        ...
        try:
            ...
            meta_prompt_data = generate_meta_prompt_for_category(

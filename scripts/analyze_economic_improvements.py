...
def demonstrate_new_capabilities():
    ...
    try:
        ...
        for channel, cost in analytics["total_costs"].items():
            if channel != "total":
                pct = (
                    (cost / analytics["total_costs"]["total"] * 100)
                    if analytics["total_costs"]["total"] > 0
                    else 0
                )
        ...
        print(
            f"  Net profit: ${analytics['total_income'] - analytics['total_costs']['total']:.2f}"
        )
        ...
        for date in sorted(analytics["by_date"].keys()):
            ...
            print(f"{date}:")
            print(
                f"  Cost: ${data['total']:.4f}, Income: ${data['income']:.2f}, "
        ...
        with open(token_costs_file, "r") as f:
            for line in f:
                ...
                if rec.get("type") == "work_income" and not rec.get("payment_awarded"):
                    ...
                    print(
                        f"  Score: {rec['evaluation_score']:.2f} (threshold: {rec['threshold']:.2f})"
                    )

...
def demo_new_format():
    ...
    try:
        ...
        print(
            f"Tasks rejected (low quality): {summary['tasks_completed'] - summary['tasks_paid']}"
        )
        ...
        print(
            f"  Net profit:  ${summary['work_income'] - summary['costs']['total']:.2f}"
        )
...
def validate_integration_points():
    ...
    print(
        f"{'✓' if has_eval_score else '✗'} add_work_income() requires evaluation_score parameter"
    )
    ...
    returns_4_values = "float, float" in str(
        return_annotation
    ) or "Tuple[bool, float, str, float]" in str(return_annotation)
    checks.append(("evaluate_artifact returns 4 values", returns_4_values))
    print(
        f"{'✓' if returns_4_values else '✗'} evaluate_artifact() returns (accepted, payment, feedback, score)"
    )

    # Check 4: Check live_agent.py integration
    live_agent_file = (
        Path(__file__).parent.parent / "livebench" / "agent" / "live_agent.py"
    )
    ...
    direct_tools_file = (
        Path(__file__).parent.parent / "livebench" / "tools" / "direct_tools.py"
    )
    ...
    print(
        f"{'✓' if has_eval_score_param else '✗'} direct_tools.py passes evaluation_score to add_work_income()"
    )
...
def validate_threshold_logic():
    ...
    try:
        ...
        for i, (score, base, expected, desc) in enumerate(test_scenarios):
            ...
            print(
                f"{status} Score {score:.4f}: ${actual:.2f} (expected ${expected:.2f}) - {desc}"
            )
        ...
        for rec in work_income_records:
            ...
            if not logic_correct or not amount_correct:
                print(
                    f"✗ Logic error in record: score={score}, threshold={threshold}, "
...
def validate_cost_channel_separation():
    ...
    try:
        tracker = EconomicTracker(
            signature="channel-test", initial_balance=1000.0, data_path=temp_dir
        )
        tracker.initialize()
        ...
        print(
            f"  LLM tokens:  ${task_costs['llm_tokens']:.6f} (expected ~${llm_cost:.6f})"
        )
        print(
            f"  Search API:  ${task_costs['search_api']:.6f} (expected ~${search_cost:.6f})"
        )
        print(
            f"  OCR API:     ${task_costs['ocr_api']:.6f} (expected ~${ocr_cost1 + ocr_cost2:.6f})"
        )
        print(
            f"  Other API:   ${task_costs['other_api']:.6f} (expected ~${other_cost:.6f})"
        )
        ...
        checks = [
            ...
            (
                abs(task_costs["search_api"] - search_cost) < tolerance,
                "Search API cost",
            ),
            (
                abs(task_costs["ocr_api"] - (ocr_cost1 + ocr_cost2)) < tolerance,
                "OCR API cost",
            ),
...
def validate_query_capabilities():
    ...
    try:
        tracker = EconomicTracker(
            signature="query-test", initial_balance=1000.0, data_path=temp_dir
        )
        tracker.initialize()

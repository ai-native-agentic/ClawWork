...
def test_basic_tracking():
    ...
    try:
        tracker = EconomicTracker(
            signature="test-agent", initial_balance=1000.0, data_path=temp_dir
        )
        tracker.initialize()
        ...
        # Track search API
        cost2 = tracker.track_api_call(
            tokens=100, price_per_1m=0.05, api_name="JINA_Search"
        )
        print(f"✓ Search API cost: ${cost2:.6f}")

        # Track OCR API
        cost3 = tracker.track_api_call(
            tokens=1000, price_per_1m=0.0417, api_name="OCR_Input"
        )
        ...
        assert (
            abs(tracker.get_balance() - expected_balance) < 0.0001
...
def test_evaluation_threshold():
    ...
    try:
        ...
        assert (
            tracker.total_work_income == 100.0
        ...
        for i in range(3):
            ...
            if i < 2:  # First 2 tasks get paid
                tracker.add_work_income(
                    amount=30.0, task_id=task_id, evaluation_score=0.8
                )
            else:  # Third task fails quality threshold
                tracker.add_work_income(
                    amount=30.0, task_id=task_id, evaluation_score=0.4
                )
    ...
    try:
        ...
        for day in range(1, 4):
            ...
            for task_num in range(2):
                ...
                tracker.add_work_income(
                    amount=40.0, task_id=task_id, evaluation_score=score
                )
        ...
        for date, costs in sorted(analytics["by_date"].items()):
            print(
                f"    - {date}: ${costs['total']:.6f} (income: ${costs['income']:.2f})"
            )
        ...
        assert (
            analytics["tasks_rejected"] == 3
        ), "Should have 3 rejected tasks (score < 0.6)"
...
def test_token_costs_file_format():
    ...
    try:
        ...
        assert (
            record_types == expected_types
        ), f"Expected {expected_types}, got {record_types}"
    ...
    try:
        ...
        for i, (score, expected_payment, desc) in enumerate(test_cases):
            ...
            actual = tracker.add_work_income(
                amount=50.0, task_id=f"task-{i}", evaluation_score=score
            )
            tracker.end_task()

            assert (
                actual == expected_payment
    ...
    try:
        ...
        for i in range(3):
            ...
            payment = tracker.add_work_income(
                amount=40.0, task_id=task_id, evaluation_score=0.7 + (i * 0.1)
            )

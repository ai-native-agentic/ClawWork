    ...
    with open(HOURLY_WAGE_FILE, "r") as f:
        ...
        for row in reader:
            ...
            if h_mean and h_mean != "*":
                try:
                    wage_data.append(
                        {"occ_title": row["OCC_TITLE"], "h_mean": float(h_mean)}
                    )
                except ValueError:
                    # Skip rows where H_MEAN can't be converted to float
                    log_message(
                        f"Warning: Skipping row with invalid H_MEAN: {row['OCC_TITLE']}"
                    )
                    continue
    log_message(
        f"Loaded {len(wage_data)} wage entries (excluding entries with missing data)"
    )
    return wage_data
...
def create_occupation_matching_prompt(
    gdpval_occupation: str, wage_occupations: List[str]
) -> str:
...
def match_occupation_to_wage(
    gdpval_occupation: str, wage_data: List[Dict[str, str]]
    ...
    try:
        ...
        matched_wage = next(
            (w for w in wage_data if w["occ_title"] == matched_title), None
        )
        ...
        if matched_wage:
            ...
            log_message(
                f"✅ Matched '{gdpval_occupation}' -> '{matched_wage['occ_title']}' (${matched_wage['h_mean']}/hr)"
            )
            return mapping
        else:
            log_message(
                f"❌ ERROR: Matched title '{matched_title}' not found in wage data"
            )
            return None
...
def create_occupation_mappings(
    occupations: List[str], wage_data: List[Dict[str, str]], output_dir: Path
...
def calculate_task_values(
    ...
    if missing_mapping_count > 0:
        log_message(
            f"⚠️  Skipped {missing_mapping_count} tasks due to missing occupation mappings"
        )
...
def generate_value_summary(task_values: List[Dict[str, Any]], output_dir: Path):
        ...
        "occupation_breakdown": [
            {
                ...
                "avg_hours_per_task": round(
                    stats["total_hours"] / stats["task_count"], 2
                ),
                "hourly_wage": round(stats["avg_hourly_wage"], 2),
                "total_value_usd": round(stats["total_value"], 2),
                "avg_value_per_task": round(
                    stats["total_value"] / stats["task_count"], 2
                ),
            }
